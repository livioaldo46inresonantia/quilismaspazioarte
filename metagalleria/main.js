...
// CIELO BLU in continuità con fascia orizzonte - non nero
scene.background = new THREE.Color(0x1a2f4a);
...
function lineIntersection(a,b,c,d){
  ...
  return new THREE.Vector3((A*(x3-x4)-(x1-x2)*B)/den, LINE_Y, (A*(z3-z4)-(z1-z2)*B)/den);
}
...
function addFloorSegment(a,b){
  ...
  mesh.rotation.y=-Math.atan2(end.z-start.z,end.x-start.x); scene.add(mesh);
...
// SOFFITTO - BLU in continuità con fascia orizzonte, non nero
const ceiling=new THREE.Mesh(new THREE.CylinderGeometry(R+0.25,R+0.25,0.08,128), new THREE.MeshStandardMaterial({color:0x1e3a5f, roughness:1}));
ceiling.position.y=CEILING_Y; scene.add(ceiling);

function addStars(){
  const starCount=600; const starsGeo=new THREE.BufferGeometry(); const pos=[];
  for(let i=0;i<starCount;i++){ const ang=Math.random()*Math.PI*2, rad=Math.random()*R*0.95, x=Math.cos(ang)*rad, z=Math.sin(ang)*rad, y=CEILING_Y-0.04-Math.random()*0.02; pos.push(x,y,z); }
  starsGeo.setAttribute('position', new THREE.Float32BufferAttribute(pos,3));
  const starsMat=new THREE.PointsMaterial({color:0xc8e0ff, size:0.022, sizeAttenuation:true, transparent:true, opacity:0.9});
  const stars=new THREE.Points(starsGeo,starsMat); scene.add(stars);
}
addStars();

scene.add(new THREE.HemisphereLight(0xffffff,0x222233,0.7));
const softLight=new THREE.DirectionalLight(0xfff1dd,0.45); softLight.position.set(3,8,4); scene.add(softLight);

const panels=[
  ['I1','I4'],['I1','I3'],['I2','I3'],['I2','I5'],[8,'I6'],[7,'I6'],[7,'I10'],['I10','I12'],['I8','I16'],[5,'I13'],['I13','I18'],['I14','I19'],[4,'I14'],['I17','I9'],[1,'I7'],[2,'I7'],[2,'I11'],['I11','I15']
];

// CORREZIONE 1: ANTRACITE NON NERO - era 0x0e0f14 troppo scuro, ora antracite vero grigio caldo
const panelMaterial=new THREE.MeshStandardMaterial({ color: 0x3a3f4a, roughness: 0.92, metalness: 0.12 });

// CORREZIONE 2: FASCE LUMINOSE LATERALI larghe come testate (8cm) - niente luce sopra
const edgeLightMat=new THREE.MeshStandardMaterial({ color:0xfff0d0, emissive:0xffe6b3, emissiveIntensity:1.4 });
const LATERAL_WIDTH = 0.08; // stessa misura testate
const LATERAL_THICK = 0.03;

function addPanel(aName,bName){
  const a=P[aName],b=P[bName]; if(!a||!b) throw new Error(`Punto pannello non definito: ${aName} – ${bName}`);
  ...
  const dx=end.x-start.x,dz=end.z-start.z; const finalLen=Math.hypot(dx,dz);
  ...
  wall.rotation.y=-Math.atan2(dz,dx); wall.castShadow=true; wall.receiveShadow=true; scene.add(wall);

  // SOLO FASCE LATERALI - larghe come testate - TOLTA luce sopra
  const sideGeo=new THREE.BoxGeometry(LATERAL_WIDTH, PANEL_HEIGHT, LATERAL_THICK);
  ...
  leftSide.position.copy(wall.position); leftSide.position.add(perp.clone().multiplyScalar(-finalLen/2 - LATERAL_WIDTH/2 + 0.02));
  leftSide.rotation.y=wall.rotation.y; scene.add(leftSide);
  
  const rightSide=new THREE.Mesh(sideGeo, edgeLightMat);
  rightSide.position.copy(wall.position); rightSide.position.add(perp.clone().multiplyScalar(finalLen/2 + LATERAL_WIDTH/2 - 0.02));
  rightSide.rotation.y=wall.rotation.y; scene.add(rightSide);

  // Luce calda diffusa dai lati
  const leftLight=new THREE.PointLight(0xffddaa, 0.9, 3.5, 2);
  leftLight.position.set(leftSide.position.x, PANEL_RAISE + PANEL_HEIGHT/2, leftSide.position.z); scene.add(leftLight);
  const rightLight=new THREE.PointLight(0xffddaa, 0.9, 3.5, 2);
  rightLight.position.set(rightSide.position.x, PANEL_RAISE + PANEL_HEIGHT/2, rightSide.position.z); scene.add(rightLight);
}
panels.forEach(([a,b])=>addPanel(a,b));

// TRIANGOLO CENTRALE VERTICALE DI SPECCHI 2.10m - MOVIMENTO / IMMAGINE / SUONO
const centerMirrorMaterial=new THREE.MeshPhysicalMaterial({
  color:0xffffff, metalness:0.95, roughness:0.08, transmission:0.02, clearcoat:1.0, clearcoatRoughness:0.05, side:THREE.DoubleSide
});
...
// SFERA GRANDISSIMA ALTISSIMA 20-30m - sorgente luce specchiante - COSCIENZA
const THIRD_EYE_RADIUS = 1.80;
const THIRD_EYE_CENTER_Y = 26.0; // altissima 20-30m come richiesto
...
for(let i=0;i<positions.count;i++){
  ...
  const variation=1+(Math.random()-0.5)*0.08; v.multiplyScalar(variation);
  positions.setXYZ(i,v.x,v.y,v.z);
}
positions.needsUpdate=true; thirdEyeGeometry.computeVertexNormals();
const thirdEyeMaterial=new THREE.MeshPhysicalMaterial({
  color:0xffffff, metalness:0.88, roughness:0.12, clearcoat:1.0, clearcoatRoughness:0.04, flatShading:true,
  emissive:0xffffff, emissiveIntensity:0.15 // leggera sorgente luce specchiante
});
const thirdEyeSphere=new THREE.Mesh(thirdEyeGeometry,thirdEyeMaterial);
thirdEyeSphere.position.copy(thirdEyeCenter); scene.add(thirdEyeSphere);

// Luce che viene dalla sfera - sorgente di luce specchiante
const sphereLight=new THREE.PointLight(0xffffff, 2.5, 80, 1.5);
sphereLight.position.copy(thirdEyeCenter); scene.add(sphereLight);

function animateSphere(dt){ thirdEyeSphere.rotation.y+=dt*0.05; thirdEyeSphere.rotation.x+=dt*0.02; }
...
const PANORAMA_DISTANCE_FROM_GALLERY=20.0, PANORAMA_HEIGHT=3.2, PANORAMA_BASE_Y=0.0, PANORAMA_RADIUS=R+PANORAMA_DISTANCE_FROM_GALLERY;
...
const panoramaBackdrop=new THREE.Mesh(panoramaGeometry,panoramaMaterial); panoramaBackdrop.position.y=PANORAMA_BASE_Y+PANORAMA_HEIGHT/2; scene.add(panoramaBackdrop);
